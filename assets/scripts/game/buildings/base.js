...
import { flyTo, getMap } from '../map/map.js';
import { roadsState, getAllLoadedRoads } from '../map/roads/roadsState.js';
import { getBuildingImage } from './building.js';
import { CHUNK_SIZE } from '../utils/chunk.js';
import { safeLoadChunk, loadVisibleRoadChunks } from '../map/roads/roads.js';
import { findClosestPointOnRoad, findClosestSegment } from '../map/roads/roadUtils.js';
...
function createBase(lat, lng) {
    ...
    baseCircle = drawBaseCircle('player_base', lat, lng, currentPlayerFaction, true);
...
export function loadOtherBase(lat, lng, pseudo, faction) {
    ...
    const isPlayerBase = faction === currentPlayerFaction;

    // Utiliser drawBaseCircle de drawOnMap.js
    const circle = drawBaseCircle(`other_${lat}_${lng}`, lat, lng, faction, isPlayerBase);
